...
export class OfficeState {
  ...
  resizeMap(newCols: number, newRows: number) {
    if (newCols <= this.cols || newRows <= this.rows) return;
    const oldCols = this.cols;
    const oldRows = this.rows;
    const newTiles = new Array(newCols * newRows).fill(255); // Fill with VOID
    for (let r = 0; r < oldRows; r++) {
      for (let c = 0; c < oldCols; c++) {
        newTiles[r * newCols + c] = this.layout.tiles[r * oldCols + c];
      }
    }
    const newLayout = {
      ...this.layout,
      ...
      rows: newRows,
      tiles: newTiles,
      tileColors: this.layout.tileColors ? new Array(newCols * newRows).fill(null) : undefined,
    };
    if (this.layout.tileColors) {
      for (let r = 0; r < oldRows; r++) {
        for (let c = 0; c < oldCols; c++) {
          newLayout.tileColors![r * newCols + c] = this.layout.tileColors[r * oldCols + c];
        }
      }
    }
  ...
  addAgent(
    ...
    if (seatId) {
      ...
      if (this.walkableTiles.length > 0) {
        // Find a walkable tile that is near the center
        const centerCol = this.layout.cols / 2;
        const centerRow = this.layout.rows / 2;
        const sortedWalkable = [...this.walkableTiles].sort((a, b) => {
        ...
        // Pick among the top 10 closest to center
        const topN = Math.min(10, sortedWalkable.length);
        spawn = sortedWalkable[Math.floor(Math.random() * topN)];
      }
